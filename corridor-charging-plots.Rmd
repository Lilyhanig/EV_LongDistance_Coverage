...
```{r}
  ...
  file.path <- here("../corridor_coverage_project/data/")                                         #here("Research/EV_NREL/data/")
...
ggplot()+geom_sf(data=counties %>% filter(STATEFP %in% c('02')) %>% mutate(Baseline.coverage = replace_na(Baseline.coverage, 0)), aes(fill=`Baseline.coverage`), colour = alpha("gray70", 1 / 2), size = 0.2)+ scale_fill_gradientn(colors=c("#fbfddf", "#e1e96b","#8da72a","#4E8E5B", "#0e758c"),limits = c(0,1))+ theme_minimal()+
  guides(fill = guide_colourbar(title = "minimum viable coverage\nlevel 2 & DC chargers\n(MD & HD truck)")) +theme(panel.grid = element_blank())
```
...
ggplot()+geom_sf(data=counties %>% filter(STATEFP %in% c('02')) %>% mutate(AFC.coverage = replace_na(AFC.coverage, 0)), aes(fill=`AFC.coverage`), colour = alpha("gray70", 1 / 2), size = 0.2)+ scale_fill_gradientn(colors=c("#fbfddf", "#e1e96b","#8da72a","#4E8E5B", "#0e758c"),limits = c(0,1))+ 
  geom_sf(data=eligible.hwys%>% filter((AFC==1), !(STFIPS %in% c(2, 60, 66, 69, 72, 78, 15))), color = '#d9d2e9')+
...
ggplot()+geom_sf(data=states %>% filter(!(STATEFP %in% c('60', '66', '69', '72', '78'))), fill = '#f7f9ff')+ 
 # geom_sf(data=st_union(eligible.hwys%>% filter(!(STFIPS %in% c(2, 60, 66, 69, 72, 78, 15)))), color="#ddeeff")+
  geom_sf(data=afdc.dcfc.proj%>% filter(!(STFIPS %in% c(2, 60, 66, 69, 72, 78, 15)), !(ID==237731)), aes(color=`Maximum Vehicle Class`), alpha=1,size= 0.8)+
  scale_color_manual(" ", #values = c("FALSE"= "#e76f51","TRUE"="#16537e"), 
                     values = c("HD"= "#e96b21","MD"="#13532e"), 
                     labels = c('Heavy-Duty', 'Medium & Heavy-Duty'))+
  theme_minimal()+theme(legend.position = c(0.25,0.13))+
